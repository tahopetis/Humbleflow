...
- Path from `humbleflow` CLI: `humbleflow init --dry-run` to discover the templates path

If the templates directory is not found, tell the human to first install humbleflow:
```bash
# Via Pi:
...
npx humbleflow init . --name "Project Name" --domains "auth,billing"
```
...
| `SPEC.md` | `./SPEC.md` | Replace `{{PROJECT_NAME}}`, `{{PROJECT_DESCRIPTION}}`, `{{CREATED_DATE}}` with today. Then append the rich content generated in Phase 2. |
...
This project works with both Pi and Claude Code:
- **Pi**: `pi install git:github.com/tahopetis/humbleflow`
- **Claude Code**: `/plugin install humbleflow@<marketplace>` or `claude --plugin-dir ./humbleflow`
